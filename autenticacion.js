...
function generarToken() {
  return "tkn_" + Math.random().toString(36).substr(2, 9);
}
...
function login(email, password) {
  ...
  if (!usuario) {
    ...
    if (usuarioExistente) {
      ...
      if (usuarioExistente.intentos >= MAX_INTENTOS_LOGIN) {
...
function validarRut(rut) {
  return typeof rut === "string" && rut.length >= 8 && rut.includes("-");
}
...
function validarRegistro(datos) {
  ...
  if (!datos.pass || datos.pass.length < 8) {
...
function registrar(datos) {
  ...
  if (emailEnUso) {
    return { ok: false, errores: ["El email ya está registrado"] };
  }
