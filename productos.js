...
function buscarPorId(id) {
  return productos.find((p) => p.id === id && p.activo) || null;
}

function estadoStock(stock) {
  if (stock === 0) return { estado: "Agotado", color: "red", alerta: true };
  if (stock <= UMBRALES_STOCK.CRITICO)
    return { estado: "Crítico", color: "orange", alerta: true };
  if (stock <= UMBRALES_STOCK.BAJO)
    return { estado: "Bajo", color: "yellow", alerta: true };
  if (stock <= UMBRALES_STOCK.NORMAL)
    return { estado: "Normal", color: "green", alerta: false };
