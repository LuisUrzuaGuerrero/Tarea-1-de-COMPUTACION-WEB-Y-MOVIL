...
function promedio(arr) {
  return arr.length > 0 ? sumar(arr) / arr.length : 0;
}
function unicos(arr) {
  return [...new Set(arr)];
}
...
function calcularEnvio({
  ...
  if (esGratis) {
    return { costo: 0, desglose: "Envío gratis" };
  }
...
function generarReporte(tipo, datos, { desde = "", hasta = "" } = {}) {
  ...
  if (tipo === "ventas") {
    ...
    datos.forEach((v) =>
      lineas.push(
        `Orden: ${v.id} | Total: ${formatearPrecio(v.total)} | Estado: ${v.estado}`,
      ),
    );
    lineas.push(
      "",
      `Total órdenes: ${datos.length}`,
      `Total ingresos: ${formatearPrecio(sumar(totales))}`,
      `Promedio: ${formatearPrecio(Math.round(promedio(totales)))}`,
      `Máximo: ${formatearPrecio(Math.max(...totales))}`,
      `Mínimo: ${formatearPrecio(Math.min(...totales))}`,
    );
  }
  ...
  if (tipo === "productos") {
    ...
    datos.forEach((p) =>
      lineas.push(
        `${p.nombre} | ${formatearPrecio(p.precio)} | Stock: ${p.stock} | Rating: ${p.rating}`,
      ),
    );
    lineas.push(
      "",
      `Total productos: ${datos.length}`,
      `Precio promedio: ${formatearPrecio(Math.round(promedio(precios)))}`,
      `Precio máximo: ${formatearPrecio(Math.max(...precios))}`,
      `Precio mínimo: ${formatearPrecio(Math.min(...precios))}`,
    );
  }
  ...
  if (tipo === "usuarios") {
    ...
    datos.forEach((u) =>
      lineas.push(
        `${u.nombre} | ${u.email} | ${u.tipo} | Puntos: ${u.puntos} | Activo: ${u.activo}`,
      ),
    );
    lineas.push(
      "",
      `Total usuarios: ${datos.length}`,
      `Puntos promedio: ${Math.round(promedio(puntos))}`,
      `Máx puntos: ${Math.max(...puntos)}`,
      `Mín puntos: ${Math.min(...puntos)}`,
    );
  }
