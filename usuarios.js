...
function actualizarPerfil(usuarioId, campos)
{
  ...
  const camposPermitidos=['nombre','telefono','direccion','ciudad','region','codPostal'];
  const camposInvalidos=Object.keys(campos).filter(c=>!camposPermitidos.includes(c));
  if (camposInvalidos.length>0) {return {ok:false, mensaje:`Campos no permitidos: ${camposInvalidos.join(', ')}`};}
...
function agregarAWishlist(usuarioId, productoId)
{
  const usuario=usuarios.find(u=>u.id===usuarioId);
  if (!usuario) {return {ok:false, mensaje:'Usuario no encontrado'};}
  if (usuario.wishlist.includes(productoId)) {return {ok:false, mensaje:'Producto ya en wishlist'};}
  usuario.wishlist.push(productoId);
  return {ok:true, mensaje:'Agregado a wishlist', wishlist:usuario.wishlist};
}
...
function quitarDeWishlist(usuarioId, productoId)
{
  ...
  if (indice===-1) {return {ok:false, mensaje:'Producto no está en wishlist'};}
  usuario.wishlist.splice(indice,1);
  return {ok:true, mensaje:'Removido de wishlist', wishlist:usuario.wishlist};
}
...
{
  if (rating<1||rating>5) {return {ok:false, mensaje:'Rating debe ser entre 1 y 5'};}
...
function darLikeResena(resenaId)
{
  ...
  if (!resena) {return {ok:false, mensaje:'Reseña no encontrada'};}
  resena.likes++;
...
{
  const indice=resenas.findIndex(r=>r.id===resenaId&&r.usuarioId===usuarioId);
  if (indice===-1) {return {ok:false, mensaje:'Reseña no encontrada o no autorizado'};}
