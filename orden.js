...
import {calcularTotal, vaciar} from './carrito.js';
import autenticacion from './autenticacion.js';
 
const IVA=0.19;
 
const INTERES_CUOTAS={2:0.02, 3:0.04, 6:0.08, 12:0.15, 24:0.28, 36:0.45};
...
function calcularPrecioFinal(precioBase, {descuentoNivel=0, descuentoCupon=0, descuentoEspecial=0, conIva=true, costoEnvio=0, cuotas=1}={})
{
  ...
  return {base:precioBase, subtotal, iva, envio:costoEnvio, total:precio, valorCuota:cuotas>1 ? precio/cuotas : precio};
}
...
function checkout(usuarioId, {metodoPago, direccion, datosTarjeta, cuotas=1}={})
{
  const usuario=usuarios.find(u=>u.id===usuarioId);
  if (!usuario) {return {ok:false, mensaje:'Usuario no encontrado'};}
  if (usuario.carrito.length===0) {return {ok:false, mensaje:'El carrito está vacío'};}
  if (!validarPago(metodoPago, datosTarjeta)) {return {ok:false, mensaje:'Datos de pago inválidos'};}
 
  const items=usuario.carrito.map(item=>{
    const producto=productos.find(p=>p.id===item.productoId);
    return {nombre:producto.nombre, cantidad:item.cantidad, precioUnitario:producto.precio, totalItem:producto.precio*item.cantidad};
  });
